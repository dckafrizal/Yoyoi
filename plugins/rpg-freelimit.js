let handler = async(m, { conn }) => {
    ...
    console.log(`__timers: ${__timers}, _timers: ${_timers}, timers: ${timers}`);
    if (new Date - user.freelimit > 79200000) {
        if (user.limit >= 1) throw '_Limit Kamu Harus 0 Untuk Mengambil Free Limit_';
        m.reply(`_Sukses Mendapatkan_\n_+${emoticon('limit')} 30 Limit_`);
        user.limit += 30;
        user.freelimit = new Date * 1;
    } else {
        m.reply(`_Silahkan Tunggu Selama_ _*${timers}*_ _Untuk Mengambil Limit Kembali..._`);
    }
};
...
function emoticon(type) {
    const emoticons = {
        'limit': '🎟️',
        // Add other types and corresponding emoticons here
    };
    console.log(`emoticon type: ${type}`);
